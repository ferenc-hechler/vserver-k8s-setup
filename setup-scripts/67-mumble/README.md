...
Edit `66-vaultwarden/values.yaml` and set the `adminToken.value` with the generated token.

### 2. Configure Domain

Update the domain in both files:
- `66-vaultwarden/values.yaml` - Set `domain` parameter
- `66-vaultwarden/vaultwarden-ingress.yaml` - Set `host` in rules and TLS sections
