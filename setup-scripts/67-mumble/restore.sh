...
# Restore script for Mumble from backup
...
echo "Mumble Restore Script"
echo "=========================="
echo ""
echo "WARNING: This will overwrite all existing Mumble data!"
echo ""
...
if [ -z "$1" ]; then
    ...
    ls -lh ./backups/mumble-backup-*.tar.gz 2>/dev/null || echo "  No backups found"
    exit 1
fi
...
kubectl wait --for=delete pod -n "$NAMESPACE" -l app.kubernetes.io/name=mumble --timeout=60s || true
...
kubectl wait --for=condition=ready pod -n "$NAMESPACE" -l app.kubernetes.io/name=mumble --timeout=120s
...
# Stop Mumble process temporarily (if possible)
echo "5. Stopping mumble process..."
kubectl exec -n "$NAMESPACE" "$POD" -- sh -c "killall mumble || true" || true
sleep 2
...
kubectl exec -n "$NAMESPACE" "$POD" -- sh -c "
    ...
    # Restore from backup
    cp -r /tmp/backup/mumble_server_config.ini /data/ 2>/dev/null || true
    cp -r /tmp/backup/mumble-server.sqlite /data/ 2>/dev/null || true
