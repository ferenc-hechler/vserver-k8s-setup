...
2023-06-06 23:13:50,101 DEBUG: Invoke endpoint. Verb: 'GET', Endpoint: 'LOGIN', Params: '{'url': 'https://conjur.vps-2.cluster-4.de', 'account': 'default'}', Data length: '0', Check errors: 'True', SSL verification metadata: '{'mode': <SslVerificationMode.TRUST_STORE: 0>, 'ca_cert_path': None}', Basic auth user: 'host/BotApp/myDemoApp', using API token: 'False', Query params: 'None', Headers: 'None', Decode token: 'True'
...
2023-06-06 23:13:50,293 DEBUG: Invoke endpoint succeeded. Duration: 187ms, Request: GET https://conjur.vps-2.cluster-4.de/authn/default/login, Response: {'status': 200, 'content length': '55'}
...
```
curl -d "490v....qsvz" -k "https://conjur.vps-2.cluster-4.de/authn/default/host%2FBotApp%2FmyDemoApp/authenticate" > /tmp/conjur_token
...
VAR_VALUE=$(curl -s -k -H "Content-Type: application/json" -H "Authorization: Token token=\"$CONT_SESSION_TOKEN\"" https://conjur.vps-2.cluster-4.de/secrets/default/variable/BotApp%2FsecretVar)
echo $VAR_VALUE
