...
helm repo update
helm upgrade --install hedgedoc-postgresql bitnami/postgresql --namespace hedgedoc --values 61-hedgedoc/psql-values.yaml

helm repo add schmitzis https://schmitzis.github.io/helm-charts/
helm repo update
helm upgrade --install hedgedoc schmitzis/hedgedoc --namespace hedgedoc --create-namespace --values 61-hedgedoc/values.yaml
